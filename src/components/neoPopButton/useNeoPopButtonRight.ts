...
export const useNeoPopButtonRight = (props: NeoPopButtonProps) => {
  const {
    titleSize = 30,
    width = 200,
    height = 100,
  ...
  const [animatedWidth] = useState<number>(0);
  const [animatedX] = useState<number>(0);
  const [animatedHeight] = useState<number>(0);
  const [animatedY] = useState<number>(0);
  const [textPosition, setTextPosition] = useState({
    x: width / 2 - titleSize,
    y: height / 2 - titleSize / 2,
  });
  const [boxShadow, setBoxShadow] = useState(0);
  const [shadowBoxWidth, setshadowBoxWidth] = useState(-20);
  const [borderPosition, setBorderPosition] = useState(8);
  ...
  const onPressOut = () => {
    ...
    setshadowBoxWidth(-20);
    setBorderPosition(10);
    setTextPosition({
      x: textPosition.x + x,
      y: textPosition.y + x,
    });
